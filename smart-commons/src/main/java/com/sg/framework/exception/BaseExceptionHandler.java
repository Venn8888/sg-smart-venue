...
public class BaseExceptionHandler {
     ...
     * @return
     */
    public static SgResponse resolveOauthException(Exception ex, String path) {
     ...
     * @return
     */
    public static SgResponse resolveException(Exception ex, String path) {
        ...
        if (className.contains("UsernameNotFoundException")) {
            ...
            if ("Bad credentials".contains(message)) {
                ...
                code = ErrorCode.ACCOUNT_LOCKED;
            } else if ("User account has expired".contains(message)) {
                code = ErrorCode.ACCOUNT_EXPIRED;
            }
            ...
            if (ErrorCode.ACCESS_DENIED_BLACK_LIMITED.getMsg().contains(message)) {
                code = ErrorCode.ACCESS_DENIED_BLACK_LIMITED;
            } else if (ErrorCode.ACCESS_DENIED_WHITE_LIMITED.getMsg().contains(message)) {
                code = ErrorCode.ACCESS_DENIED_WHITE_LIMITED;
            } else if (ErrorCode.ACCESS_DENIED_AUTHORITY_EXPIRED.getMsg().contains(message)) {
                code = ErrorCode.ACCESS_DENIED_AUTHORITY_EXPIRED;
            } else if (ErrorCode.ACCESS_DENIED_UPDATING.getMsg().contains(message)) {
                code = ErrorCode.ACCESS_DENIED_UPDATING;
            } else if (ErrorCode.ACCESS_DENIED_DISABLED.getMsg().contains(message)) {
                code = ErrorCode.ACCESS_DENIED_DISABLED;
            } else if (ErrorCode.ACCESS_DENIED_NOT_OPEN.getMsg().contains(message)) {
                code = ErrorCode.ACCESS_DENIED_NOT_OPEN;
            }
        } else if (className.contains("HttpMessageNotReadableException") || className.contains("TypeMismatchException")
     ...
     * @return
     */
    private static SgResponse buildBody(Exception exception, ErrorCode resultCode, String path, int httpStatus) {
        if (resultCode == null) {
            resultCode = ErrorCode.ERROR;
        }
        SgResponse resultBody = SgResponse.failed(resultCode, httpStatus, path);
