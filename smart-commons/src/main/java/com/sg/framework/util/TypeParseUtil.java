...
import com.sg.framework.base.BaseType;
...
public final class TypeParseUtil {
	...
	public static final Object convert(Object value, String type, String format) {
		...
		} else if ("String".equalsIgnoreCase(type) || BaseType.STRING.equalsIgnoreCase(type)) {
			return value.toString();
		} else {
			throw new BusinessException(String.format(support, value.getClass().getName(), type));
		}
	}
	...
	private static Object bool2Obj(Object value, String type) {
		...
		if ("Boolean".equalsIgnoreCase(type) || BaseType.BOOLEAN.equalsIgnoreCase(type)) {
			return bol;
		} else if ("String".equalsIgnoreCase(type) || BaseType.STRING.equalsIgnoreCase(type)) {
			return bol.toString();
		} else if ("Integer".equalsIgnoreCase(type) || BaseType.INTEGER.equalsIgnoreCase(type)) {
			if (bol.booleanValue()) {
			...
			throw new BusinessException(String.format(support, fromType, type));
		}
	}
	...
	private static Object time2Obj(Object value, String type, String format) {
		...
		if ("String".equalsIgnoreCase(type) || BaseType.STRING.equalsIgnoreCase(type)) {
		...
		} else if ("Date".equalsIgnoreCase(type) || BaseType.DATE.equalsIgnoreCase(type)) {
			return new java.util.Date(tme.getTime());
		} else if ("java.sql.Date".equalsIgnoreCase(type)) {
			return new Date(tme.getTime());
		} else if ("Time".equalsIgnoreCase(type) || BaseType.TIME.equalsIgnoreCase(type)) {
			return new Time(tme.getTime());
		} else if ("Timestamp".equalsIgnoreCase(type) || BaseType.TIMESTAMP.equalsIgnoreCase(type)) {
			return value;
		} else {
			throw new BusinessException(String.format(support, fromType, type));
		}
	...
	private static Object sqlDate2Obj(Object value, String type, String format) {
		...
		if ("String".equalsIgnoreCase(type) || BaseType.STRING.equalsIgnoreCase(type)) {
			...
			throw new BusinessException("Conversion from " + fromType + " to " + type + " not currently supported");
		} else if ("Timestamp".equalsIgnoreCase(type) || BaseType.TIMESTAMP.equalsIgnoreCase(type)) {
	...
	private static Object intr2Obj(Object value, String type, Locale locale) {
		...
		} else if ("Double".equalsIgnoreCase(type) || BaseType.DOUBLE.equalsIgnoreCase(type)) {
			return new Double(intgr.doubleValue());
		} else if ("Float".equalsIgnoreCase(type) || BaseType.FLOAT.equalsIgnoreCase(type)) {
			return new Float(intgr.floatValue());
		} else if ("BigDecimal".equalsIgnoreCase(type) || BaseType.BIGDECIMAL.equalsIgnoreCase(type)) {
		...
		} else if ("Long".equalsIgnoreCase(type) || BaseType.LONG.equalsIgnoreCase(type)) {
	...
	private static Object decimal2Obj(Object value, String type, Locale locale) {
		...
		} else if ("Integer".equals(type) || BaseType.INTEGER.equalsIgnoreCase(type)) {
	...
	private static Object string2Object(Object value, String type, String format, Locale locale) {
		...
		try {
			if ("String".equalsIgnoreCase(type) || BaseType.STRING.equalsIgnoreCase(type)) {
				...
				return null;
			} else if ("Boolean".equalsIgnoreCase(type) || BaseType.BOOLEAN.equals(type)) {
				...
				if (format == null || format.length() == 0)
					try {
						...
						try {
							...
							throw new BusinessException(String.format(message, str, type), e);
						}
					}
				SimpleDateFormat sdf = new SimpleDateFormat(format);
				java.util.Date fieldDate = sdf.parse(str);
