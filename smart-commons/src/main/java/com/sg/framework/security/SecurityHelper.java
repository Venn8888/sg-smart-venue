...
import com.sg.config.AppConstants;
...
public class SecurityHelper {
     ...
     * @param openUser
     */
    public static void updateOpenUser(TokenStore tokenStore, BaseUserDetails openUser) {
        ...
        if (accessTokens != null && !accessTokens.isEmpty()) {
            for (OAuth2AccessToken accessToken : accessTokens) {
                ...
                BeanUtil.setProperty(authentication, "principal", openUser);
                // 重新保存
    ...
    public static void updateOpenClientAuthorities(TokenStore tokenStore,String clientId,Collection<? extends GrantedAuthority> authorities) {
        ...
        if (accessTokens != null && !accessTokens.isEmpty()) {
            ...
            while (iterator.hasNext()) {
                ...
                // 由于没有set方法,使用反射机制强制赋值
                BeanUtil.setProperty(oAuth2Authentication, "authorities", authorities);
                // 重新保存
