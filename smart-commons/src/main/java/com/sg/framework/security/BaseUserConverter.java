...
import com.sg.framework.util.BeanUtil;
...
public class BaseUserConverter extends DefaultUserAuthenticationConverter {
    ...
    private Object converter(Map<String, ?> map) {
        ...
        BaseUserDetails auth = BeanUtil.mapToObject(params, BaseUserDetails.class);
        if (params.get(USERNAME) != null) {
            auth.setUsername(params.get(USERNAME).toString());
        }
        if (params.get(Constants.OPEN_ID) != null) {
            auth.setUserId(params.get(Constants.OPEN_ID).toString());
        }
//        if (params.get(Constants.DOMAIN) != null) {
//            auth.setDomain(params.get(Constants.DOMAIN).toString());
//        }
